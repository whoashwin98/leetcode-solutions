...
public:
    bool isPossible(int m, int n, int t, vector<vector<int>>& cells) {
        ...
        for (int i = 0; i < t; i++) {
            // mark cells from the given list as blocked
    ...
    int latestDayToCross(int row, int col, vector<vector<int>>& cells) {
        ...
        while (left < right - 1) {
            // calculate the mid-day
            int mid = left + (right - left) / 2; 
            if (isPossible(row, col, mid, cells)) {
                // update the left pointer to search in the upper half
                left = mid; 
                // update the latest possible day
                latestPossibleDay = mid; 
            } 
            else {
                // update the right pointer to search in the lower half
                right = mid; 
            }
        }
