...
public:
    int solve(string text1, string text2, int i1, int i2,vector<vector<int>>& dp) {
        ...
        int found = -1e9;
        if(text1[i1] == text2[i2]) found = 1 + solve(text1, text2, i1-1, i2-1, dp);

        // if not found, either move i1 or i2, and take the max out of both
        int notFound = 0 + max(solve(text1, text2, i1-1, i2, dp), solve(text1, text2, i1, i2-1, dp));

        return dp[i1][i2] = max(found, notFound);
    }
